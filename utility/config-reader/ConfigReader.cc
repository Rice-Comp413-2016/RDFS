...
namespace config_reader {
	...
	void ConfigReader::InitHDFSDefaults() {
		processConfigFile(HDFS_DEFAULTS_CONFIG);
	}

	void ConfigReader::processConfigFile(const char* filename) {
		xml_document doc;
		xml_parse_result result = doc.load_file(filename);
		if (!result) {
			LOG(ERROR) << CLASS_NAME << "XML [" << filename << "] parsed with errors, attr value: [" <<
